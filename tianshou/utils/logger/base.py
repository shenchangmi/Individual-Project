...
class BaseLogger(ABC):
    ...
    def log_rew_data(self, policy: dict, step: int) -> None:
        """
        rew
        """
        # policy.ret_rms.mean
        # if policy > 0:
        if step - self.last_log_rew_step >= self.train_interval:
            log_data = {
                "ret/mean": policy.ret_rms.mean,
                "ret/var": policy.ret_rms.var,
                "ret/count": policy.ret_rms.count,
            }
            self.write("ret/env_step", step, log_data)
            self.last_log_rew_step = step
