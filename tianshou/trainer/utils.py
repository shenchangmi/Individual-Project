...
def test_episode(
    ...
    if type(collector) == Collector:
        ...
        collector.reset_buffer()
        result = collector.collect(n_episode=n_episode)
    elif type(collector) == list:
        results = []
        for env in collector:
            env = DummyVectorEnv(
                [lambda: env for _ in range(1)]
            )
            env.seed(1)
            env_collector = Collector(policy, env)
            env_collector.reset_stat()
            env_collector.reset_env()
            env_collector.reset_buffer()
            result = env_collector.collect(n_episode=1)
            results.append(result)
        
        result= {
                "n/ep": len(collector),
                "n/st": np.mean([res['n/st'] for res in results]),
                "rews": np.array([res['rews'][0] for res in results]),
                "lens": np.array([res['lens'][0] for res in results]),
                "idxs": [i for i in range(len(collector))],
                "rew": np.mean(np.array([res['rews'] for res in results])),
                "len": np.mean(np.array([res['lens'] for res in results])),
                "rew_std": np.std(np.array([res['rews'] for res in results])),
                "len_std":  np.std(np.array([res['lens'] for res in results])),
            }
    else:
        print('bad env')
...
def gather_info(
    ...
    if test_collector is not None and type(test_collector)==Collector:
    ...
    if train_collector is not None and type(test_collector)==Collector:
